...
simul_data <- function(n_breeders = 1000, # number of pairs
  ...
  return(list(capt_sess = df_site, 
              ...
              R_et = mean(n_eggs),
              mu_star = mean(ld_dates*fitness),
              var_star = var(ld_dates*fitness)))
      ...
      for(q in 1:n_rep){
        ...
        if(class(md_sim) == "try-error"){
          ...
          df_rec[[k]] <- data.frame(
            ...
            # R_hat_sel = ((2*pinf)/(1-pinf))*sqrt(((omega_til^2)+1)/(omega_til^2))*exp(((shiftopt^2)/((((omega_til^2)+1)/(omega_til^2))*(((pi^2)*b^2)/3)))/(2*(omega_til^2+1))),
            
             R_hat_sel = n_eggs*sqrt(((omega_til^2)+1)/(omega_til^2))*exp(((shiftopt^2/var_bar)/((((omega_til^2)+1)/(omega_til^2))*(((pi^2)*b^2)/3)))/(2*(omega_til^2+1))),
...
df_simul3 %>% 
  ...
  # geom_function(fun = function(w){n_eggs},
