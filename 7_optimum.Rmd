...
The estimated mean laying date after selection is `r mean(md_df$ld_after_selection)` while the simulated one (before selection) is `r data1$mean_ld_year[1,2]`. 
...
```
The estimated variance of laying date after selection is `r mean(md_df$est_var_ld)` while the simulated one (before selection) is 49. 
